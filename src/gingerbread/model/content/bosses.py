...
BOSSES: Final[dict[str, BossSpec]] = {
    ...
    "slime": BossSpec(
        ...
        hp=46, speed=24.0, radius=28.0, sugar=18,
        weakness="thunder",
        traits=("mud_trail",),
        params={"mud_every": 0.5, "mud_radius": 34.0,
                "mud_slow": 0.5, "mud_life": 7.0},
        colour=(196, 108, 172),
        phases=(
            BossPhase(until_hp=0.55, behaviour="flank",
                      params={"orbit": 170.0, "patience": 99.0},
                      summons=(("splitter", 6.0),),
                      announce="它拖著身體爬過來，地上留下一路糖漿。"),
            BossPhase(until_hp=0.0, behaviour="flank",
                      params={"orbit": 110.0, "patience": 3.0},
                      summons=(("splitter", 4.0), ("child", 5.0)),
                      announce="它裂開了——最小的那塊，得靠雷。"),
        )),
    ...
    "mist_reaper": BossSpec(
        ...
        phases=(
            BossPhase(until_hp=0.6, behaviour="flank",
                      ...
                      summons=(("faint", 6.0),), fog=0.55,
                      announce="濃霧壓下來。就算換了燈罩也看不遠。"),
            BossPhase(until_hp=0.0, behaviour="charge",
                      summons=(("faint", 4.0), ("digger", 7.0)), fog=0.4,
                      announce="霧更濃了。要嘛救妹妹，要嘛打它。"),
        )),

    # 第五夜 — 教「水能清路，也能逼它現形」
    "ash_hob": BossSpec(
        key="ash_hob", name="灰燼灶鬼", title="第五夜",
        hp=64, speed=30.0, radius=28.0, sugar=24,
        weakness="water",
        traits=("mud_trail",),
        params={"mud_every": 0.35, "mud_radius": 30.0,
                "mud_slow": 0.85, "mud_life": 5.0},
        colour=(214, 96, 44),
        phases=(
            BossPhase(until_hp=0.55, behaviour="flank",
                      params={"orbit": 150.0, "patience": 99.0},
                      summons=(("bomber", 6.0),),
                      announce="它邊走邊噴火。用水沖出一條路。"),
            BossPhase(until_hp=0.0, behaviour="charge",
    ...
    "moonfall": BossSpec(
        ...
        colour=(146, 118, 206),
        phases=(
            BossPhase(until_hp=0.6, behaviour="standoff",
                      params={"standoff": 300.0, "windup": 2.2,
                              "reload": 2.6, "shot_speed": 200.0,
                              "shot_damage": 1},
                      summons=(("armoured", 8.0),),
                      announce="隕石總是砸向你站的地方。"),
            BossPhase(until_hp=0.0, behaviour="standoff",
                      params={"standoff": 220.0, "windup": 1.4,
                              "reload": 1.6, "shot_speed": 240.0},
